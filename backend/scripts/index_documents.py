...
def index_docs():
    ...
    logger.info(f"Embedding Deployement: {os.getenv('AZURE_OPENAI_EMBEDDING_DEPLOYEMENT', 'text-embedding-3-small')}")
    ...
    try:
        ...
        embeddings = AzureOpenAIEmbeddings(
            azure_deployment=os.getenv("AZURE_OPENAI_EMBEDDING_DEPLOYEMENT", "text-embeding-3-small"),
    ...
    try:
        ...
        logger.error(f"Failed to initialize Azzure Search: {e}")
    ...
    # 10. Upload to Azure
    if all_splits:
        logger.info(f"Uploading {len(all_splits)} chunks to Azure to Azure AI Search Index '{index_name}'...")
        try:
